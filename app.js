...
const updatePage = async () => {
  ...
  const filteredList = fruitsArray.filter(fruit => fruit.family === 'Rosaceae').map(fruit => ({
    name: fruit.name,
    family: fruit.family,
}));
console.log(filteredList);
  ...
  if (!fruitContainer) {
      console.error("Could not find 'fruit-Container' element.");
      return;
  }
  ...
  newElement.innerHTML = 'Fruits belonging to the Rosaceae family:';
  
  const ulElement = document.createElement('ul');

  filteredList.forEach(fruit => {
    const liElement = document.createElement('li');
    liElement.innerText = fruit.name;
